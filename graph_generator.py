...
from utils_graph_generator import create_descriptors, identify_functional_groups, CanonicalAtomFeaturizer, CanonicalBondFeaturizer, RandomScaffoldSplitter, RandomSplitter, RandomStratifiedSplitter_m

data_names = ['bbbp']  # [name_data_1, name_data_2, ...] like ['qm7', 'amu']
generation_models_rows = {'Hierarchical_Quotient' : [10]}  # {name_model : idx_row_graph_gens} like {'Hierarchical_Quotient' : [11,10,1,0], "Quotient_complement" : [0]} 
...
for name_data in data_names:
    for name_model in generation_models_rows.keys():
        for idx_row_graph_gen in generation_models_rows[name_model]:
            ...
            for split in splits: 
                ...
                print( '\n\n\n\n ========================= \n  Generating', name_model, 'with row number:', idx_row_graph_gen, 'graphs', \
                    'for', split, 'seeds:', generation_seeds, 'splitted',\
                         name_data, 'dataset is now started! \n ========================= \n\n\n\n')
                ...
                list_gen_seeds = []
                for seed in generation_seeds: 
                    path_save= current_dir+"data/graph/"+name_data+"/"+split+"_"+str(seed)+"/"+name_final_zip
                    if not os.path.exists(path_save):
                        ...
                        name_final = kind+name_node_feature
                        name_final_zip = name_final+".zip"
                        name_node_feats_zip = name_data+ "_node"+name_node_feature+".zip"
                        ...
                        if name_model == "Hierarchical_Quotient": 
                            def add_features(smiles, mol_dgl_graph):
                                ...
                                if num_quotient == 1: 
                                    if num_layers_including_quotient==3:
                                        # mol_dgl_graph_q1.ndata["qn1"]=torch.full((mol_dgl_graph_q1.num_nodes(),1), -1).to(torch.int32)
                                        # mol_dgl_graph_q1.edata["qe1"]=torch.full((mol_dgl_graph_q1.num_edges(),1), -1).to(torch.float32)
                                        # mol_dgl_graph.ndata["qn2"]=torch.full((mol_dgl_graph.num_nodes(),1), 0).to(torch.int32)
                                        # mol_dgl_graph.edata["qe2"]=torch.full((mol_dgl_graph.num_edges(),1), 0).to(torch.float32)
                                    ...
                                    elif num_layers_including_quotient ==2:  
